...
bool ProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex& sourceParent) const
{
  ...
  //# = #
  QString abbreviation = sourceModel()->data(sourceModel()->index(sourceRow, 0, sourceParent)).toString();
  QString firstName    = sourceModel()->data(sourceModel()->index(sourceRow, 1, sourceParent)).toString();
  QString lastName     = sourceModel()->data(sourceModel()->index(sourceRow, 2, sourceParent)).toString();
  QString phoneNumber  = sourceModel()->data(sourceModel()->index(sourceRow, 3, sourceParent)).toString();
  QString room         = sourceModel()->data(sourceModel()->index(sourceRow, 4, sourceParent)).toString();

  bool searchStringContained = false;
  // TODO how can I get the setFilterFixedString() directly (without storing in member)?
  searchStringContained |= abbreviation.contains(m_searchString, Qt::CaseInsensitive);
